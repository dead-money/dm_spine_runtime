...
- [x] 5 — constraint solvers: IK (5a, 1-bone + 2-bone), Transform (5b, four World/Local × Absolute/Relative variants + `updateAppliedTransform`), Path (5c, four SpacingModes × three RotateModes + constant-speed curve arc-length), Physics (5d, damped-spring integrator with fixed timestep). Capture harness + goldens regenerated with the full constraint pipeline. **Parity status:** all 25 rigs run without panics; 20/25 setup-pose fixtures match at 1e-4, 5 rigs diverge (mix-and-match, sack, spineboy, stretchyman, tank) — debug follow-ups queued. 9/35 animation samples match; the per-rig delta summary is logged by the golden_animation test. 5-rig divergence focuses around Transform/Path constraint corner cases; spineboy's `rear-foot-target` (Transform) is the lead follow-up.
